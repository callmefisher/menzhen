...
- **诊疗记录** — 就诊记录管理，支持主诉（textarea）、脉象（搜索下拉 + AI 回退 + 详情卡片）、舌象（图片上传 + 描述 + AI 分析），诊断模板自动同步患者信息/主诉/脉象/舌象，附件上传（图片/音频/视频），诊断标签移动端自适应
- **电子处方** — 基于方剂一键开方、编辑药物及剂量、多列排版打印（每 10 味一列），支持草药和中成药双区域（中成药自动查询方剂功效/主治和库存），医嘱分行展示，开方时显示库存提示
...
- **易理（I Ching / 卦象管理）** — 64卦完整数据库（卦辞/六爻爻辞/传文/中医应用/关联卦），支持按卦名和八卦分类搜索，管理员可新增/编辑/删除
- **经络穴位 3D 可视化** — Three.js 人体模型（男/女双模型），20 条经络路径 + 367 穴位标记，BVH 表面投影，点击穴位查看详情
- **经络详情** — 特殊穴位属性（五输穴、原穴、络穴等）、教学视频、出处介绍，管理员可编辑

### 库存管理

- **药物库存** — 支持本草（克）和成药（盒）两种分类，CRUD + 分页搜索 + 分类筛选，含货架号管理（默认 H1）
- **入库管理** — 单个入库（累加库存量）+ 批量入库（已有药物累加、新药物自动创建），支持货架号
- **库存预警** — 前端定时扫描低库存药物，红色高亮提示，显示货架号便于定位补货，支持屏蔽/全局阈值配置（存 localStorage）
