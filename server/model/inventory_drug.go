...
// The unique index includes deleted_at so that soft-deleted records don't block re-creation.
type InventoryDrug struct {
	BaseModel
	TenantID       uint64         `gorm:"column:tenant_id;not null;index;uniqueIndex:idx_tenant_name" json:"tenant_id"`
	Name           string         `gorm:"column:name;type:varchar(100);not null;uniqueIndex:idx_tenant_name" json:"name"`
	DeletedAt      gorm.DeletedAt `gorm:"uniqueIndex:idx_tenant_name" json:"-"`
